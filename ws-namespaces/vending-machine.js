...
const signToken = db.query(
  'SELECT "token" FROM "machines" WHERE "name" like \'vending-machine-%\' LIMIT 1;',
);
const secretKeyVendingMachine =
  process.env.TOKEN_SECRET_KEY_VENDING_MACHINE || signToken.rows[0].token;
...
ioMachine.of("/vending-machine").on("connection", async (socket) => {
  socket.on("stateUpdate", async (data) => {
    ...
    if (state === "idle") {
      await db.query(
        'UPDATE "machines" SET "busy" = false WHERE "name" = (SELECT "name" FROM "machines" WHERE "name" LIKE \'vending-machine%\' LIMIT 1);',
        (err) => {
          if (err) {
            console.error(err);
            return;
      ...
      if (operation > 0) {
        let product = await db.query(
          'SELECT "product" as "id" FROM "operations" WHERE "id" = $1',
          [operation],
          (err) => {
            if (err) {
              console.error(err);
        ...
        await db.query(
          'UPDATE "stock" SET "quantity" = "quantity" - 1 WHERE "product" = $1;',
          [product.rows[0].id],
          (err) => {
            if (err) {
              console.error(err);
      ...
      await db.query(
        'UPDATE "machines" SET "busy" = true WHERE "name" = (SELECT "name" FROM "machines" WHERE "name" LIKE \'vending-machine%\' LIMIT 1);',
        (err) => {
          if (err) {
            console.error(err);
            return;
